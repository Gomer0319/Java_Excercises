...
public class ProductController {
    public void addProduct() {// Add a product to the product list
        // Have a message to add a product with name, category, price per unit, if it
        // needs prescription, supplier. Name should be unique, if there is a product
        // with the same name, the system should throw an exception
        // The product ID should be auto-generated by the system. If there is a product
        // with the same ID, the system should throw an exception.
        // If there is a new product, the system should add it to the product list, with
        // the product ID + 1 from the previous product.
        ...
        while (keepAdding) {
            ...
            if (addAnother.equalsIgnoreCase("n")) {
                // If the user does not want to add another product, the system should exit the
                // loop and a message should be displayed
    ...
    public void addToInventory() {// Add to inventory
        ...
        System.out.println("\n--------Add to Inventory--------");
        // Find item either by name or product ID. If the product is not found, the
        ...
        for (Product product : Product.productList) {
            if (product.getProductId().equals(productId)) {
                ...
                String invoiceNumber = Utility.getString("Invoice Number: ").trim();
                if (invoiceNumber.isEmpty() || invoiceNumber.equals("0")) {
                    System.out.println("No valid invoice number entered. No items added to inventory.");
                    return;
                }
                ...
                // Generate an ID for the inventory transaction, with a character concatenated
                // to the product ID (example: RT + the generated ID (RT0001))
    ...
    public void updateProduct() {// Update product
        ...
        // Find item by ID. If the product is not found, the system should throw an
        // exception.
    ...
    public void offsetInventoryTransaction() {
        ...
        for (Product transaction : Product.inventoryList) {
            if (transaction.getInventoryTransactionId().equals(transactionId)) {
                ...
                System.out.println("\nTransaction Matched:");
                System.out.printf(" %-15s %-10s %-10s %-30s %-10.2f %-10.2f %-20s %-20s\n",
                ...
                double offsetQuantity = Utility
                        .getDouble("Enter offset quantity (negative for deduction, positive for addition): ");
                Utility.userInput.nextLine();
