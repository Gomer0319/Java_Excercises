...
public class ToysCart implements Cart {
    private int ID;
    ...
    public static List<ToysCart> toysCart = new ArrayList<>();
        ...
        for (ToysCart cartItem : toysCart) {
            if (cartItem.getProductID() == ID) {
                ...
                System.out.println("\nProduct (" + quantity + ") " + cartItem.getProductName()
                        + " added to Toys Cart successfully.");
                found = true;
                break;
            }
        }
        ...
        if (!found) {
            for (Product product : Toys.toys) {
                if (product.getProductID() == ID) {
                    ...
                    ToysCart newCartItem = new ToysCart(ID, product.getProductName(), product.getProductPrice(),
                            quantity);
        ...
        if (toysCart.size() == 0) {
            ...
            for (int i = 0; i < toysCart.size(); i++) {
                ...
                System.out.printf("|| %-20s || %-40s || %-20d || %30.2f ||%n",
                        toysCart.get(i).getCategory() + toysCart.get(i).getProductID(),
                        toysCart.get(i).getProductName(), toysCart.get(i).getProductQty(),
                        toysCart.get(i).getProductPrice() * toysCart.get(i).getProductQty());
            }
        }
    }
