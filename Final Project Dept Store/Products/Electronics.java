...
public class Electronics implements Product {
    ...
    public static void addProduct(int ID, String name, double price, int quantity) {
        // Check if there is a product ID and name already exist, if there are, add the
        // quantity of the existing product. If
        // the ID already exist but with a different name, display an error message that
        // the ID is already in use. If the name
        // already exist but with a different ID, display an error message that the name
        // is already in use. If it is a new product,
        // add it to the list.
        ...
        for (int i = 0; i < electronics.size(); i++) {
            ...
            if (product.getProductID() == ID && product.getProductName().equals(name)
    ...
    public static void editProduct(int ID) {
        ...
        // Check if there are any products in the electronics category based on the
        // product ID
        for (int i = 0; i < electronics.size(); i++) {
            Electronics product = electronics.get(i);
        ...
        for (int i = 0; i < electronics.size(); i++) {
            ...
            if (product.getProductID() == ID) {
                ...
                System.out.println("Product with ID " + ID + " with name " + product.getProductName()
                        + " has been deleted successfully from Electronics category.");
                return;
            }
        }
        // If the product is not found, print a message
    ...
    // Method to view or display products from viewProducts class for electronics
    // category
    public static void viewProducts() {
        ...
        if (electronics.size() == 0) {
            ...
            System.out.println(
                    "----------------------------------------------------------------------------------------------------------------------------------------------------------");
            // Loop through the electronics list and display the products, one by one, one
